...
const UserdataView = () => {
    ...
    const [username, setUsername] = useState(userSession.IsAuth ? userSession.user.userName : "Username");
    ...
    return (<div className="mx-4 text-indigo-950">
        <div className={edit ? "hidden" : "flex flex-col"}>
            <div className="bg-indigo-100 rounded-md px-2">
                <p className="p-1 text-ellipsis overflow-hidden">{userSession.IsAuth ? userSession.user.userName : "Username"}</p>
                <p className="p-1 text-ellipsis overflow-hidden">{userSession.IsAuth ? userSession.user.email : "E-mail"}</p>
                <p className="p-1 text-ellipsis overflow-hidden">Birthday: {userSession.IsAuth ? dayjs(userSession.user.birthDateUtc).format('MMM DD YYYY') : "Unknown"}</p>
            </div>
