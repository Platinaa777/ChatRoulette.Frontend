        ...
        <div className='w-full text-indigo-950'>
            {
                rating.length === 0 ? <p>Wow... empty</p> :
                    <ul className='w-full mt-3'>
                        {
                            rating.map((person, idx) => (
                                <li
                                    key={idx}
                                    className={`cursor-pointer w-full flex justify-between items-center flex-row relative rounded-md p-3 ${person.userName === userSession.profile.userName ? "border-2 border-blue-500" : "border-blue-800 border"
                                        } mb-1 bg-indigo-50`}>
