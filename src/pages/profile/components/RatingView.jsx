        ...
        <div className='w-full text-indigo-950'>
            {<ul className='w-full mt-3'>
                {rating.map((person, idx) => (<li
                    key={idx}
                    className={`cursor-pointer w-full flex justify-between items-center flex-row relative rounded-md p-3 ${person.userName === userSession.profile.userName ? "border-2 border-blue-500" : "border-blue-800 border"} mb-1 bg-indigo-50`}
                >
                ...
                {!(rating.some(person => person.userName === userSession.profile.userName)) && <>
                        ...
                        <li className='cursor-pointer mt-3 w-full flex justify-between items-center flex-row relative rounded p-3 border border-blue-800 mb-1 bg-blue-50'>
                            <div className='flex items-center'>
                                <img alt="" src={userSession.profile.avatar} width={40} height={40} className="rounded overflow-hidden" />
                                <h3 className="ml-4 text-l font-medium leading-5">
                                    {userSession.profile.userName}
                                </h3>
                            </div>
                            <h3 className='text-l font-medium leading-5'>{userSession.profile.rating}</h3>
                        </li>
                    </>
                }
            </ul>}
